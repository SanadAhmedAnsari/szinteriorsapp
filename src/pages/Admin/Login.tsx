  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (error.code === "auth/popup-blocked") {
        toast.error(
          "Popup blocked. Allow popups for this site, or use email/password below.",
          { duration: 8000 },
        );
      } else if (error.code === "auth/unauthorized-domain") {
        toast.error(
          "This domain is not authorized in Firebase. Use email/password login instead.",
          { duration: 8000 },
        );
          ...
          {mode === "email" ? (
            <form onSubmit={handleEmailLogin} className="space-y-6 text-left">
              <div className="space-y-2">
                ...
                <div className="relative">
                  <Mail
                    size={16}
                    className="absolute left-0 top-3.5 text-stone-300"
                  />
                  <input
                    type="email"
                    value={email}
            ...
            <div className="space-y-6">
              <p className="text-stone-600 text-sm leading-relaxed">
                Sign in with your authorized Google account. Make sure popups
                are allowed and{" "}
