      ...
      <div className="flex items-end justify-between">
        <div>
          ...
          <p className="mt-2 text-stone-500 text-sm uppercase tracking-widest">
            Manage your project reels and tutorials
          </p>
        </div>
        <button
          onClick={() => {
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-3">
          {videos.map((video) => (
            <div
              key={video.id}
              className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-stone-100 space-y-6"
            >
              <div className="relative aspect-[9/16] overflow-hidden rounded-2xl bg-stone-100">
                <img
                  src={
                    video.thumbnail ||
                    `https://picsum.photos/seed/${video.id}/800/1400`
                  }
                  alt={video.title}
                  ...
                  <div className="space-y-2">
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setFormData({
                          ...formData,
                          type: e.target.value as "project" | "tutorial",
                        })
                      }
                      className="w-full border-b border-stone-200 bg-transparent py-3 text-stone-900 focus:border-stone-900 focus:outline-none transition-colors"
                    >
